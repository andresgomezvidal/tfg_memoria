...
\linej
\linej
Additionally a detection for the DLLs it uses could be implemented. They were found by making Sysmon report every DLL and filtering the events from Reg. They were later included in the configuration:
\begin{lstlisting}[style=xml]
<ImageLoad onmatch="include">
	...
	<ImageLoaded condition="contains">kernelbase.dll</ImageLoaded>
...
\end{lstlisting}
\linej
In the case tested each DLL was loaded once by each of the 3 Reg commands. Of course an attacker could only use one of them, therefore there is one rule for each pair of DLLs:
\lstinputlisting[style=xml]{scripts/rules_reg_grouping.xml}
