...
\subsubsection{Exploit 11: Retrieval of NTDS.DIT with NinjaCopy}
%%fixes needed
...
\lstinputlisting[style=PS]{src/distributed_logins_winrs.ps1}

\linej
The rule 60205 in Wazuh only works for failed attempts comming from the same ip address, therefore it fails to detect this. A similar rule can solve this, just by checking the same user is targeted and by changing the <same\_field> requirement to <not\_same\_field> for the ip address:
\begin{lstlisting}[style=xml]
...
It needs to be run as Administrator to be able to query the security log.
The OU name and the hour range are set in the script by hand, but could also be automated.
%%<wodle name="command">
%	%<disabled>no</disabled>
%	%<tag>remoteOU</tag>
%	%<command>powershell C:\\Users\\Public\\Documents\\OU.ps1</command>
...
\linej
\lstinputlisting[style=PS,caption=Remote script to find logins on unusual hours for the OU users,captionpos=b]{src/OU.ps1}
\linej
...
\lstinputlisting[style=xml]{src/rules_OU.xml}
