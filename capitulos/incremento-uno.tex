The title of these increments are \textit{\IncrementoUno} and \textit{\IncrementoDos}. Due to reasons stated later in \ref{increment_explanation} these increments that were planned to be implemented individually were actually done together.
This chapter satisfies the next non-functional requirements: RNF-01, RNF-02, RNF-03, RNF-04, RNF-05 and RNF-08.
\linej
\linej
...
\linej
\linej
In this laboratory we disabled the antivirus and Windows Defender to avoid unnecessary delays.
...
To keep this simple only the basics are explained about the techniques used in the examples for a Golden Ticket attack. Because some of the exploits are similar they are numbered for easier identification. The scripts in the next exploits were used to understand the different ways to perform Golden Ticket attacks and during the tests to try to detect them.
Of course there are more ways to generate a Golden Ticket (and some are much more harder to detect) and it is possible to combine several of the next exploits or change some of their steps.
\linej
...
\subsubsection{Exploit 1.1: Local Mimikatz in DC}
...
\linej
\lstinputlisting[style=PS,caption=Script to generate and inject a Golden Ticket,captionpos=b]{src/ticket_detection/genticket.ps1}
\linej
...
\linej
\lstinputlisting[style=txt,caption=Example of the contents of the output file,captionpos=b]{src/ticket_detection/output.txt}
\linej
...
\subsubsection{Exploit 1.2: Mimikatz from memory in DC} \label{invoke-Mimikatz}
...
\linej
\lstinputlisting[style=PS,caption=Script to run Mimikatz only in memory and inject a Golden Ticket,captionpos=b]{src/ticket_detection/genticket_mem.ps1}
\linej
...
\linej
\lstinputlisting[style=PS,caption=Script to run Mimikatz with DCSync,captionpos=b]{src/ticket_detection/genticket_dcsync_mimikatz.ps1}
...
\subsubsection{Exploit 1.4: DCSync with Kiwi}
...
\linej
\lstinputlisting[style=ruby,caption=Part 1 of the remote DCSync exploit automation,captionpos=b]{src/ticket_detection/metasploit_p1_kiwi.rc}
\linej
...
\linej
\lstinputlisting[style=ruby,caption=Part 2 of the remote DCSync exploit automation,captionpos=b]{src/ticket_detection/metasploit_p2_kiwi.rc}
\begin{figure}[H]
	\centering
...
\linej
\linej
Of course there is no real need to use Metasploit to get a remote shell to run Mimikatz. The attacker could use SSH, run remote commands with \textit{psexec} or use the Windows Remote Shell. But some of these need to be enabled and they would not be much different from the previous examples.

\subsubsection{Exploit 1.5: Hashdump with Meterpreter}
...
\linej
\lstinputlisting[style=ruby,caption=Part 1 of the remote Hashdump exploit automation,captionpos=b]{src/ticket_detection/metasploit_p1_hashdump.rc}
\linej
...
\linej
\lstinputlisting[style=ruby,caption=Part 2 of the remote Hashdump exploit automation,captionpos=b]{src/ticket_detection/metasploit_p2_hashdump.rc}
\begin{figure}[H]
	\centering
...
\linej
\linej
%The techniques to detect or avoid detection evolve with each other over time.
...
\end{itemize}
\linej
The real useful addition to the Windows built-in events is having Sysmon\cite{sysmon} in each of the monitored Windows computers. They can be combined to identify attackers better\cite{detection_events}.
\linej
...
\linej
\lstinputlisting[style=xml,caption=Sysmon monitoring with event 7 for certain DLLs,captionpos=b]{src/ticket_detection/sysmon_event_7.xml}
\linej
On the manager side the next rules are needed:
\linej
\lstinputlisting[style=xml,caption=Rules for suspecting a Mimikatz execution as a group of events,captionpos=b]{src/ticket_detection/rules_event_7_mimikatz_alternative.xml}
...
\linej
The conversion to JSON was done manually to add information and because the \textit{ConvertTo-Json} cmdlet does not work as expected.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{figuras/klist_normal.png}
...
\linej
\lstinputlisting[style=xml,caption=Rules to detect a suspicious expiration age from the report of the klist script,captionpos=b]{src/ticket_detection/rules_klist.xml}
\linej
...
\lstinputlisting[style=xml]{src/ticket_detection/sysmon_event_1_klist.xml}
\linej
And checking with Wazuh if the option \textit{purge} was used:
\lstinputlisting[style=xml]{src/ticket_detection/rules_klist_purge.xml}
...
\subsubsection{Exploit 2.1: Retrieval of NTDS.DIT with ntdsutil}
...
\lstinputlisting[style=xml]{src/credential_extraction/sysmon_event_1_ntdsutil.xml}
\linej
And alerts set with Wazuh:
\label{ntdsutil_detection}
\lstinputlisting[style=xml]{src/credential_extraction/rules_ntds.xml}
\linej
The first rule is the parent, it filters windows events with the \textit{ntds} string.
\linej
The second rule detects the ntdsutil executable. This signature is more useful than usual because it is a built-in tool. The third matches \textit{ntds} for the command line, which is not really reliable.
\linej
...
\subsubsection{Exploit 2.2: Storing registry hives with Reg}
...
\lstinputlisting[style=PS]{src/credential_extraction/reg.ps1}
\linej
Reg is not detected as malware because it is a built-in tool in Windows. With Sysmon we can report the execution of Reg with the event 1, reporting the creation of a process:
\lstinputlisting[style=xml]{src/credential_extraction/sysmon_event_1_reg.xml}
\linej
And Wazuh to trigger an alert:
\lstinputlisting[style=xml]{src/credential_extraction/rules_reg.xml}
\linej
...
\lstinputlisting[style=PS]{src/credential_extraction/procdump.ps1}
\linej
...
\lstinputlisting[style=PS]{src/credential_extraction/pd.ps1}

\subsubsection{Exploit 2.5: Dump of LSASS with Minidump}
Minidump is a script from the PowerSploit Post-Explotation Framework\cite{powersploit}. It can be combined with the \textit{Get-Process} built-in to dump the process into a file:
\lstinputlisting[style=PS]{src/credential_extraction/minidump.ps1}
...
\lstinputlisting[style=PS]{src/credential_extraction/NinjaCopy.ps1}
\linej
Or to copy the NTDS.DIT of the DC in this laboratory to a no-DC computer:
\lstinputlisting[style=PS]{src/credential_extraction/NinjaCopy_from_DC.ps1}
\linej
...
\lstinputlisting[style=xml]{src/credential_extraction/sysmon_event_1_9_NinjaCopy.xml}
\linej
\lstinputlisting[style=xml,caption=Rules for detecting NinjaCopy,captionpos=b]{src/credential_extraction/rules_ninjacopy.xml}
\linej
...
\lstinputlisting[style=xml]{src/credential_extraction/sysmon_event_7_ninjacopy_import.xml}
\linej
These 8 DLLs are detected by 4 rules to reach the minimum frequency:
\lstinputlisting[style=xml]{src/credential_extraction/rules_ninjacopy_import.xml}
\linej
This detection works the same with both local and remote cases.
\linej
False positives were found when attempting to log in remotely multiple consecutive times, as described on the part about brute force reverse logins on page \pageref{reverse_login}.
...
\linej
\lstinputlisting[style=xml,caption=Sysmon monitoring with event 10 for LSASS reads,captionpos=b]{src/credential_extraction/sysmon_event_10_lsass.xml}
\linej
...
\linej
\lstinputlisting[style=xml,caption=Rules to detect unusual values of grantedAccess,captionpos=b]{src/credential_extraction/rules_lsass.xml}
\linej
...
\lstinputlisting[style=xml]{src/credential_extraction/sysmon_event_8_lsass.xml}
\linej
As for Wazuh just a rule to detect the event of type 8 and the LSASS executable is enough:
\lstinputlisting[style=xml]{src/credential_extraction/rules_lsass_event_8.xml}
\linej
...
PowerShell provides a built-in option to execute commands from an encoded string. This can be used to bypass signature matching. Of course the attacker does not need to encode it in the same machine, that would be a risk for the key string to be detected.
\linej
\lstinputlisting[style=PS,caption=Encoding and executing ntdsutil,captionpos=b]{src/powershell_detection/powershell_enc.ps1}
\linej
Data about the execution of PowerShell can be obtained with the event 1 of Sysmon:
\lstinputlisting[style=xml]{src/powershell_detection/sysmon_event_1.xml}
\linej
...
\lstinputlisting[style=xml,]{src/powershell_detection/rules_powershell_enc_latex.xml}
\linej
...
\linej
\linej
The encoded command used in this case triggered the alerts for too long CommandLine (100100) and the previous alerts to detect the use of ntdsutil in \ref{ntdsutil_detection}: CommandLine with ntdsutil.exe (300011) and suspicious database events (300013, 300014 and 300015).
...
For example the previous Invoke-Mimikatz\ref{invoke-Mimikatz} and Invoke-NinjaCopy\ref{invoke-NinjaCopy} methods stop working because they rely on reflective DLL loading.
\linej
...
\linej
The tool tested was Invoke-Mimikatz, which was previously used in order to test the detection of Mimikatz in \ref{invoke-Mimikatz}.
\linej
...
All of them have been shown in this project, along with mitigation and/or detection techniques.
\linej
...
\linej
In all these cases the code is run on the no-DC, which authenticates against the DC, using the built-in command \textit{winrs}\cite{winrs}. Here this command is used to specify the remote computer to log in, the user, the password and the command to run after a successful authentication, in that order.
\linej
\linej
...
\lstinputlisting[style=PS]{src/logins/reverse_logins_winrs.ps1}
\linej
...
\linej
\lstinputlisting[style=xml,captionpos=b,caption=Rule 60205 of Wazuh]{src/logins/rule_60205.xml}
...
\linej
\linej
The rule 60205 in Wazuh only works for failed attempts coming from the same IP address, therefore it fails to detect this. A similar rule can solve this, just by checking the same user is targeted and by changing the \textit{same\_field} requirement to \textit{not\_same\_field} for the IP address:
\lstinputlisting[style=xml,captionpos=b,caption=Rule 60205 of Wazuh]{src/logins/rule_60205_alternative.xml}
\linej
...
Even so we can not always rely in it and there are situations where it is interesting to monitor if there are attempts to log in on unusual hours.
...
As with the case of the klist detection (on page \pageref{klist_detection}) a PowerShell script was created \ref{lst:OU}\ and a remote command was set \ref{lst:OU_wodle}.
...
More sophisticated methods can be used, but it is important to remember that brute force logins are not efficient when compared to offline cracking.
There is always a risk of users having insecure passwords, which can be reduced by enforcing and teaching good password policies.
\linej
