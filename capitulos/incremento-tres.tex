...
\begin{enumerate}
	...
	\item Show a message asking for Bitcoin payment.
\end{enumerate}
\begin{figure}[H]
	\centering
	\includegraphics[width=.9\textwidth]{figuras/Bitcoin_Dharma.png}
	\caption{Dharma asking for payment in Bitcoins}
\end{figure}
\linej
...
Next is a summary of the problems and their causes in this tests:
\begin{itemize}
	\item The main problem in this case was that the resource usage was so high that any simple command took much more time than normal.
This made the alerts suffer a delay before reaching the manager, because the buffer of the agent was too small to handle so many events toguether.
The active response was too slow to act before the encryption was complete or almost complete, making it useless.
\linej
The solution was to use an early but reliable malware detection, which was not used before because it was not necessary.
The detection of the program as malware by Windows Defender was almost instantaneous and its events include the process responsible, making it easy to extract.
...
\begin{table}[H]
	\begin{tabularx}{\textwidth}{|l|X|}
		...
		62103 & Windows Defender's action against potentially unwanted software.\\ \hline
