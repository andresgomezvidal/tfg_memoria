...
These can be effective against basic automated malware, but a smart attacker can overcome them pretty easily.
		...
		\item As a last resort, turning off the computer can stop the encryption process and critical data can be retrieved from RAM using special software.
			%Unfortunately there are cases where shutting down does not stop the encryption process, leaving the computer even more vulnerable.
	\end{enumerate}
\end{enumerate}
\linej
...
%\linej
%To measure the response speed 10 test were made with the test script \ref{lst:ransomware_basic}\ resulting in an average of 5.7 seconds from the start to the stop of this script.
%Of course this does not offer any kind of guarantee that it will work in a real scenario or that this performance is good or bad, this only serves as an reference in the lab environment.
%TODO
