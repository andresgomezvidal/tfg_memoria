...
\lstinputlisting[style=PS,caption=Script to encrypt files and cipher the passphrase with RSA,captionpos=b]{src/ransomware_basic.ps1}
\linej
...
\end{lstlisting}
\linej
Sysmon events can be used to monitor changes in folders too, for example monitoring the folder \textit{C:\temp\}:
\begin{lstlisting}[style=xml]
...
\end{lstlisting}
\linej
%TODO explicar que no pueden detectar borrado



\linej
The main advantage of using Windows Defender for this detection is that it allows the accesses to be blocked (in Block mode).
It also provides some abstraction to the end user, because Windows decides which programs are legitimate and which are not (leaving whitelisting aside).
Even though Wazuh has the active response feature it is fundamentally slower because it adds multiple steps to the direct response Windows Defender offers in block mode.
Fast local action is more suited to antivirus than IDS, therefore it makes sense to turn to Windows Defender for this feature.
\linej
The disadvantages of using Windows Defender are that logs do not include the filename (just the folder path) and that it is an external program (one of the most targeted by attackers).
\linej
Fortunately there is no restriction to use either Windows Defender or Sysmon events, and both can be used, resulting in more resource usage.
It is important to note that if the changes are blocked only events of type 1123 are generated.
\linej
For the execution of this script (with 4 .txt files) Windows Defender reported 6 events of type 1124 and Sysmon 5 of type 11 (4 creations of .txt.enc and symmetric.key).
This makes sense because events 2 and 9 are triggered by change of creation time and raw access of the drive (a basic attempt to avoid detection) respectively.
\linej
\linej
%TODO false postives
	%sysmon es un cristo y hace falta active response con decoys
	%en windows defender depende de granuralizar los comandos de whitelist
	%explicar decoys con active response y checksum (file integrity monitoring Wazuh https://documentation.wazuh.com/current/user-manual/capabilities/file-integrity/index.html)
	%en cada carpeta en la que te interese monitorizar los archivos añadir un decoy que compruebas con active response de Wazuh en caso de eventos sospechosos
	%windows defender tiene una serie de carpetas que monitoriza por defecto y el resto aparecen en Computer\HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\Controlled Folder Access\ProtectedFolders
	%para todas esas carpetas se puede copiar un archivo o un archivo de cada extensión aleatoriamente y monitorizar el checksum para asegurar que no cambia

%TODO events events_46_56_X \cite{events_46_56_X}
...
%no se detecta el borrado de shadow copies por sysmon, ni windows defender, ni por los eventos 465X-466X, porque no aparecen como carpetas en el volumen



%TODO Mitigation
%%MITIGATION
%\cite{ransomware_digital_extortion}\cite{ransomware_oReilly}
